...
export function AccountSelector({
  ...
  const toggleAccount = (accountId: string, e: React.MouseEvent) => {
    ...
    setAccounts((prev) => {
      ...
      const newSelectedAccounts = newAccounts.filter(
        (account) => account.selected
      );
      if (
        newSelectedAccounts.length === 0 ||
        newSelectedAccounts.length === newAccounts.length
      ) {
  ...
  const selectSingleAccount = (account: Account) => {
    if (selectedAccount?.id === account.id) {
      ...
      setAccounts((prev) =>
        prev.map((a) => ({ ...a, selected: a.id === account.id }))
      );
    }
    setIsExpanded(false);
  };
  ...
  const getHeaderText = () => {
    ...
    if (selectedAccounts.length === 1)
      return (
        selectedAccounts[0].type.charAt(0).toUpperCase() +
        selectedAccounts[0].type.slice(1)
      );
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-2">
      <motion.div className="bg-white rounded-lg shadow-sm border" layout>
        {/* Header */}
        <motion.button
          className={cn(
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <span className="font-medium">
              {formatCurrency(
                selectedAccounts.length > 0 && !allSelected
            ...
            {accounts.map((account) => (
              <motion.div
                ...
                className={cn(
                  ...
                  account.selected &&
                    !allSelected &&
                    "bg-gray-100 rounded-md my-1 mx-2"
                )}
